...
export function makePrompt(descriptor: PromptDescriptor): PendingPrompt {
	return {
		...
		(descriptor.kind === "input" || descriptor.kind === "editor") &&
...
export function hilAbortError(signal: AbortSignal): Error {
	return signal.reason instanceof Error ? signal.reason : new Error("atomic-workflows: HIL aborted");
}
...
export function normalizeUIContext(adapter: WorkflowUIAdapter | undefined): WorkflowUIContext {
	...
	return {
		input(prompt) {
			return typeof adapter.input === "function" ? adapter.input.call(adapter, prompt) : unavailable.input(prompt);
		},
		confirm(message) {
...
export function readinessResultMeansAdvance(result: unknown): boolean {
	...
	return candidates.some(
		(candidate) =>
			typeof candidate === "string" && candidate.trim().toLowerCase() === READINESS_GATE_ADVANCE_NORMALIZED,
	);
}
