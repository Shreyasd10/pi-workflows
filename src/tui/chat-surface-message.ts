	...
	switch (payload.kind) {
		...
		case "status": {
			...
			return [
				rendered,
				"",
				...payload.runs.map((run) => `run id: ${run.id} · workflow: ${run.name} · status: ${run.status}`),
			].join("\n");
		}
		case "list": {
...
function formatWorkflowEntryPlain(entry: WorkflowListEntry): string {
	const required = entry.inputs.filter((input) => input.required === true).map((input) => input.name);
	const optional = entry.inputs.filter((input) => input.required !== true).map((input) => input.name);
	const inputs = [
...
function makeComponent(payload: ChatSurfacePayload, theme: GraphTheme): CardComponent {
...
function renderPayload(payload: ChatSurfacePayload, theme: GraphTheme, width: number, now: number): string {
	switch (payload.kind) {
		case "dispatch":
