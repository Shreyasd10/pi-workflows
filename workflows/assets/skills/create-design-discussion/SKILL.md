...
2. **Check for related task content**:
   - if a path in `docs/design-discussions/` is mentioned, use Bash(`ls docs/design-discussions/`)
   - **IMPORTANT** DO NOT USE search or glob or grep, or `ls -l` (lowercase L), as docs/design-discussions/ may be a symlink and those tools don't traverse symlinks
...
5. **Write the design discussion** to `docs/design-discussions/YYYY-MM-DD-design-discussion-DESCRIPTION.md`
   - First, check the task directory in your system prompt. If you don't see it, find the task directory: `ls docs/design-discussions/` | grep -i "eng-XXXX"`
   - If the directory doesn't exist, create: `docs/design-discussions/`
   - Format: `YYYY-MM-DD-design-discussion-DESCRIPTION.md` where NN is a zero-padded chronological index and DESCRIPTION is a 2-4 word kebab-case slug
   ...
   - Directory naming:
     - With ticket: `docs/design-discussions/`
     - Without ticket: `docs/design-discussions/`
...
`Read(references/design_discussion_final_answer.md)`

If hook context says all design questions have been resolved, read the resolved template instead:

`Read(references/design_discussion_final_answer_resolved.md)`
