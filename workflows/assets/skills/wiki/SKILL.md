...
Dispatch **`codebase-locator`** for this pass when the repo is large enough that sending the full path list back to the orchestrator would be wasteful. Ask it for the categorized file paths grouped by dossier (the same shape its prompt defines). For very large repos, use the locator and the wiki's own read-only survey; do not persist a profile as part of wiki generation.
...
After both passes, walk every top-level source directory (and one level below) to check for subsystems you missed. If the repo is large, dispatch **`codebase-locator`** first to surface any existing `docs/research/`, `docs/plans/`, `docs/context/codebase/`, or `docs/prd/` material that names subsystems — these are authoritative seeds. Then for each directory that contains its own service, module, or feature, decide:
...
**Source B: Directory enumeration.** For each lens that applies to the repo, list every subdirectory. Dispatch **`codebase-locator`** with a "list every directory under X" prompt so the path enumeration runs in an isolated context window:
...
**Every event, era, and milestone must include a date or month** (e.g., "Mar 2023", "Q4 2024"). Derive dates from git revision timestamps, tag dates, and file creation dates. If an exact date isn't available, use the month of the earliest relevant revision.
...
The sidebar ordering is critical for navigation. Every page must appear in its defined position.
...
- The `pageOrder` array in the meta file (see Section 4) must exactly follow this ordering. It controls the sidebar display order.
...
- For large repos (50+ source directories or 10+ distinct subsystems), lean toward splitting pages rather than cramming. A 3000-word page covering an entire subsystem is less useful than three focused pages covering its distinct aspects. Page authors decide whether to create sub-pages based on what they find in the code.
...
Page generation uses a top-level agent for orchestration and foundation pages, then delegates domain pages to sub-agents for depth and parallelism. If sub-agents are not available in the runtime, the orchestrator writes each page itself.

### Available agents (this workspace)

This workspace ships a runtime-neutral agent fleet under `agents/`. The orchestrator dispatches them by job, not by name. Load the prompt file for the agent you are dispatching and read its `description` and instructions before sending it work.

| Agent prompt | File | Use for |
|---|---|---|
| `workspace-locator` | `agents/workspace-locator.md` | Step 1 only — confirming the repo root when the input path is ambiguous |
| `codebase-locator` | `agents/codebase-locator.md` | Survey Pass 2 + coverage cross-check — WHERE files live, returns categorized paths |
| `codebase-analyzer` | `agents/codebase-analyzer.md` | Step 4 a — explain HOW a chosen subsystem works with file:line refs |
| `codebase-pattern-finder` | `agents/codebase-pattern-finder.md` | Step 4 a — find existing patterns and examples to model new work after |
| `codebase-locator` | `agents/codebase-locator.md` | Step 1 surveys for any existing docs/ research, plans, PRDs worth citing |
| `codebase-analyzer` | `agents/codebase-analyzer.md` | Step 1 to extract the highest-value insights from the most relevant docs |
| `cross-repo-contract-analyzer` | `agents/cross-repo-contract-analyzer.md` | Step 4 conditional — surfaces API/event/schema edges when the wiki covers more than one repo |
| `web-search-researcher` | `agents/web-search-researcher.md` | Step 4 conditional — only when the survey flagged external docs/APIs needing research |

All of these are **documentarians** — they describe what exists without suggesting changes. `implementer` is intentionally excluded here; the wiki writes markdown, not code.

If your runtime has no subagent primitive, the orchestrator reads each agent's prompt and runs its job inline. The output format and rules are encoded in the prompt files.
...
2. PLAN (orchestrator)
   ...
   Produce: page plan (per-page briefs)
        │
        ▼
3. FOUNDATION PAGES (orchestrator, sequential)
   Write: overview/*, how-to-contribute/patterns-and-conventions
   Establish shared vocabulary and conventions
        │
        ├────────────────────────────────────────┐
        ▼                                        ▼
4a. DOMAIN PAGES (sub-agents, parallel)   4b. DATA PAGES (sub-agents, parallel)
    ...
    + sub-pages if warranted                   |
        ...
        ├────────────────────────────────────────┘
        ▼
5. REMAINING PAGES (sub-agents in parallel,
   or orchestrator if no sub-agents)
...
6. ASSEMBLY (orchestrator)
   Cross-link audit, write meta file
```

### Step 2: Planning and delegation

After the survey, the orchestrator produces a **page plan** — a structured list of every page the wiki will contain. For each page, the plan includes:
...
**Criticality guidelines:** Pages covering apps, packages, or features with large codebases, high churn, or central architectural roles are strong candidates for dedicated agents. Examples: a CLI with 50+ source files, a core library imported by most other packages, a feature that spans 5+ directories. The orchestrator uses judgment from the survey — these are guidelines, not hard rules.
...
The orchestrator writes these pages sequentially before any sub-agents run:
...
**4b. Data pages** — run in parallel with domain pages since they only need git history and source file structure. Use plain git log and file scans for `by-the-numbers`, `lore`, and `fun-facts`. If the runtime has no subagent primitive, the orchestrator derives these directly from git.
...
After all domain pages complete, spawn sub-agents for:
...
These pages can now cross-reference domain pages since they're complete.

### Step 6: Assembly

The orchestrator does a final pass:

- Audit cross-links between pages (fix broken references, add missing links)
- Write the meta file with final page list and ordering
...
see [API authentication](docs/api/authentication.md example path).
```
...
After generating all pages, create a meta file in the wiki directory root. The `pageOrder` array is critical -- it controls the display order of pages in the wiki sidebar. List every generated file path in the exact order you want them to appear. Without this, pages sort alphabetically.

Use the name `.wiki-meta.json`. Example shape:
...
The wiki is a directory of markdown files that can be hosted by any markdown-aware viewer (MkDocs, Docusaurus, GitHub Pages, an internal docs site, or an LLM viewer).

### Standard publish (local wiki directory)

By default, leave the wiki directory on disk at the path agreed with the user (e.g., `./droid-wiki/` or `<repo>/docs/wiki/`). The target must not be a coordinator docs directory, a coordinator profile path, or `<repo>/docs/context/codebase/`. The user can then commit it, push it, or hand it to a static-site generator.

```bash
# Example: copy/sync into a publish location
rsync -av --delete ./wiki/ ./public/docs/
```

If the runtime provides a dedicated upload helper, use it:

```bash
# Replace with the runtime's actual upload command. Examples:
droid wiki-upload --repo-url "$REPO_URL" --wiki-dir ./wiki
# or
wiki-publish --repo-url "$REPO_URL" --wiki-dir ./wiki
```

Arguments to the upload helper, if used:
...
### Remote-only publish (no local copy)

When the user wants to generate the wiki without leaving files on disk (e.g., "don't leave files locally"):

1. Create a temporary directory: `WIKI_TMPDIR=$(mktemp -d)`
2. Write all wiki files into the temporary directory instead of the target wiki directory
3. Invoke the upload helper with `--cleanup` (or equivalent) to remove the temp directory after success
4. If the upload fails, the temp directory is preserved so the user can retry

The `--cleanup` flag tells the CLI to delete the wiki directory after a successful upload. If the upload fails, the directory is preserved so the user can retry.
...
1. **Cut inflated significance.** Do not write "serves as a testament to," "pivotal role in the evolving landscape," "setting the stage for," "underscores the importance of." Just state what the thing does.
...
- Every `.md` file must start with a level-1 heading (`# Title`). Upload tools typically extract the title from this heading.
- Every directory must contain an `index.md`.
- File names use lowercase with hyphens. No spaces, no uppercase.
