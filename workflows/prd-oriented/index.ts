...
export default workflow({
	name: "PRD-Oriented",
	description:
		"First-class PRD/technical-design/outline/implementation workflow with verbatim skill contracts, pi-task delegation, human review gates, and bounded fresh-session handoffs.",
	inputs: {
		task: Type.String({ description: "Task description or path to the task context." }),
		include_research: Type.Boolean({ default: false, description: "Run research questions and research before the PRD." }),
	...
	run: async (ctx) => {
		const host = await createDeliveryHost(ctx, "prd-oriented");
		return runDeliveryGraph(
			host,
			prdOrientedGraph(ctx.inputs.include_research === true, ctx.inputs.detailed_plan === true),
		);
	},
});
